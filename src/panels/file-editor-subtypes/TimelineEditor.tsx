...
export function TimelineClip(props: {
  ...
  let [currentlyEditingClip, setOnEditClip, editClip] = useAppStore((state) => [
    state.currentlyEditingClip,
    state.setOnEditClip,
    state.editClip,
  ]);
...
function TimelineTime(props: { time: number; style?: React.CSSProperties }) {
  if (props.time < 60)
    return (
      <span className="timeline-time" style={props.style}>
        {Math.floor(props.time * 100 + 0.00000001) / 100}s
      </span>
    );
  if (props.time < 3600)
    return (
      <span className="timeline-time" style={props.style}>
        {Math.floor(props.time / 60)}m
        {(Math.floor(props.time * 100 + 0.00000001) / 100) % 60}s
      </span>
    );
  return (
    <span className="timeline-time" style={props.style}>
      {Math.floor(props.time / 3600)}h{Math.floor(props.time / 60) % 60}m
      {(Math.floor(props.time * 100 + 0.00000001) / 100) % 60}s
    </span>
  );
}
...
export function TimelineNumbers(props: {
  ...
  if (rect) {
    ...
    for (
      let i = Math.floor(props.start / step) * step;
      i < props.end;
      i += step
    ) {
      numbers.push(
        <TimelineTime
          ...
          style={{
            ...
            top: "0px",
            left: `${
              ((i / (props.end - props.start)) * rect.width) / durationFactor
            }px`,
          }}
        ></TimelineTime>
      );
...
export function TimelineEditor(props: { uuid: string }) {
  const [files, setFile, setCurrentTimelineTime] = useAppStore((state) => [
    state.state.files,
    state.setFile,
    state.setCurrentTimelineTime,
  ]);
  ...
  if (!maybeParsedFile.success) {
    return (
      <p>Error: File claims to be a Timeline but is in the wrong format.</p>
    );
  }
  ...
  const resetTimeElapsed = useAnimationFrame<[boolean, number]>(
    ([isPlaying, playingStartTime], timeElapsed) => {
      if (isPlaying) {
    ...
    <div>
      ...
      <div
        ...
        onKeyDown={(e) => {
          ...
          let scrollLeftTimeSpace =
            (e.currentTarget.scrollLeft / rect.width) * range;
          ...
          e.currentTarget.scrollLeft =
            (newScrollLeft / (factor * range)) * rect.width;
