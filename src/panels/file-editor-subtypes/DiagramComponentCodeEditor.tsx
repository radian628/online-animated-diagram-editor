...
function SettingEditor(props: {
  ...
  return (
    <div>
        ...
        onChange={(e) => {
          props.setSetting({
            ...
            type: e.currentTarget
              .value as JSDrawable["settings"][number]["type"],
          });
        }}
      >
      ...
      <button onClick={(e) => props.deleteSetting()}>
        <IoClose></IoClose>
      </button>
    </div>
  );
      ...
      {props.settings.map((s, i) => {
        return (
          <SettingEditor
            ...
            setSetting={(s) => {
              props.setSettings(
                props.settings.map((s2, j) => (j == i ? s : s2))
              );
            }}
            deleteSetting={() => {
...
export function DiagramComponentCodeEditor(props: { uuid: string }) {
  const [files, setFile] = useAppStore((state) => [
    state.state.files,
    state.setFile,
  ]);
  const file = files[props.uuid];

  const onPerFrameUpdateChange = React.useCallback(
    (value: string) => {
  ...
  const onFixedUpdateChange = React.useCallback(
    (value: string) => {
      console.log("value: ", value);
      if (!parsedFile) return;
      setFile(
        props.uuid,
        JSON.stringify({ ...parsedFile, onFixedUpdate: value })
      );
    },
    [props.uuid, file]
  );
  ...
  if (!maybeParsedFile.success) {
    ...
    return (
      <p>
        Error: File claims to be a Diagram Component but is in the wrong format.
      </p>
    );
  }
  const parsedFile = maybeParsedFile.data;

  const [perFrameLocalCopy, setPerFrameLocalCopy] = React.useState<string>(
    parsedFile.onUpdate
  );
  const [fixedLocalCopy, setFixedLocalCopy] = React.useState<string>(
    parsedFile.onFixedUpdate
  );
