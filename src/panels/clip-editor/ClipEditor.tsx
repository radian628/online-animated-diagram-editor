...
export function StringSetting(props: {
  ...
  return (
    <div>
      ...
      <StringInput val={props.val} setVal={props.setVal}></StringInput>
    </div>
  );
}
...
export function NumberSetting(props: {
  ...
  return (
    <div>
      <label>{props.settingName}</label>
      <NumberInput val={props.val} setVal={props.setVal}></NumberInput>
    </div>
  );
}

export function ClipEditor() {
  const [currentlyEditingClip, setOnEditClip, editClip, files] = useAppStore(
    (state) => [
            ...
            case "string":
              return (
                <StringSetting
                  key={setting.varName}
                  val={
                    typeof currentlyEditingClip.settings[setting.varName] ==
                    "string"
                      ? (currentlyEditingClip.settings[
                          setting.varName
                        ] as string)
